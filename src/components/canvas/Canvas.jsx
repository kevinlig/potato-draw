...
export default class Canvas extends React.Component {
    constructor(props) {
        ...
        this.syncCanvasUp = this.syncCanvasUp.bind(this);
    ...
    connectDatabase() {
        ...
        this.database.ref('/draw/render').on('value', (snapshot) => {
            const value = snapshot.val();
            if (value !== this.state.render) {
                this.syncCanvasDown();
            }
        });

        this.fabric.on('object:modified', this.syncCanvasUp);
        this.fabric.on('path:created', this.syncCanvasUp);
        this.fabric.on('text:changed', this.syncCanvasUp);
    }

    syncCanvasDown() {
        if (!this.fabric || !this.database) {
            return;
        }

        this.database.ref('/draw').once('value', (snapshot) => {
            const value = snapshot.val();
            this.setState({
                render: value.render
            }, () => {
                this.fabric.loadFromDatalessJSON(value.data, () => {
    ...
    syncCanvasUp() {
        ...
        const render = shortid.generate();

        this.setState({
            render,
            inFlight: true
        }, () => {
            // this.database.ref('/draw').set({
            //     data: JSON.stringify(this.fabric.toDatalessJSON()),
            //     render
            // })
            // .then(()=> {
            //     console.log("sent");
            ...
            // temporarily switching to REST API due to Safari bug with Websockets
            const cancel =  CancelToken.source();
            const promise = Axios({
                method: 'put',
                url: '/draw.json',
                baseURL: this.state.dbURL,
                data: {
                    data: JSON.stringify(this.fabric.toDatalessJSON()),
                    render
                },
